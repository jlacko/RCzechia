...
The Kartierung der Flora Mitteleuropas (KFME) grid is a commonly used technique in biogeography of the Central Europe. It uses a grid of 10" longitude by 6" latitude, with cells numbered from north to south and west to east.
...
* placing it in a KFME grid cell (via `sf::st_intersection`)
* plotting the outcome – both as a grid cell and exact location – on a map


```{r ctverce, echo = T, eval = T, message = F, warning = F, out.width = "100%",fig.asp = 0.7, dpi = 100}
library(RCzechia)
library(ggplot2)
library(dplyr)
library(sf)

obec <- "Telč" # a Czech location
...
ggplot() +
  ...
  geom_sf(data = KFME_grid() %>% filter(ctverec == ctverec_id), # a specific KFME cell
