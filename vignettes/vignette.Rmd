...
Population of the Czech Republic as of 2025-01-01, per commune (obec). The results from CZSO [DataStat](https://data.csu.gov.cz/datastat) platform can be easily accessed from the comfort of your R session using the recently published API interface.
...
# map metrics - population dataset, using technical ids (instead of labels)
src <- RCzechia::datastat_dataset("OBY02A", "id") |>
  subset(CasR == "2025" & POHL2 == "0") |> # year 2025, both sexes
  select(uzemi = UZ01234596C, hodnota = value)

# municipal polygons
obecni_data <- RCzechia::obce_polygony() |>
  inner_join(src, by = c("KOD_OBEC" = "uzemi")) |> # linking by key
  mutate(poparea = hodnota / sf::st_area(geometry) |> units::set_units("km2")) # population relative to area

# report results
ggplot(data = obecni_data) +
  geom_sf(aes(fill = units::drop_units(poparea)), colour = NA) + # ggplot does not like units objects
  geom_sf(data = RCzechia::republika("low"), color = "gray30", fill = NA) +
  scale_fill_viridis_c(labels = scales::comma, trans = "log2") +
  labs(title = "Czech population density",
       fill = "population\nper km²") +
  theme_bw() +
  theme(legend.text = element_text(hjust = 1),
        legend.title = element_text(hjust = 0.5))
## Warning in scale_fill_viridis_c(labels = scales::comma, trans = "log2"): log-2
...
Since Stamen Toner basemap no longer sparkles joy I have found a new favorite - the [Positron by CartoDB](https://docs.carto.com/carto-user-manual/maps/basemaps).
...
leaflet() |>
  ...
  addPolygons(data = podklad,
              ...
              popup = ~paste0(uzemi_txt, ": nezaměstnanost ", hodnota, "%"),
              fillOpacity = 0.75,
              color = NA)
```
<p align="center">
...
vysledky <- "https://www.volby.cz/pls/senat/se1111?xjazyk=CZ&xdatum=20201002&xv=7&xt=2" |>
  ...
  html_element(xpath = "//*[@id=\"se1111_t1\"]") |>  # get the table by its xpath
  html_table() |> # convert table from HTML to tibble dormat
